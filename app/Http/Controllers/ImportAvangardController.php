...
class ImportAvangardController extends Controller
{
    /**
     * @var string
     */
    ...
    {
        $sites = Site::whereIn('id', [6690, 6435])->whereNotNull('token')->with(['request'])->get();
        if (count($sites) > 0) {
            foreach ($sites as $site) {
                ...
                try {
                    ...
                    // Log the exception
                    Log::error("Request Exception Avangard pv: " . $e->getMessage());
                }
                ...
                if (is_array($items) && count($items) > 0) {
                    foreach ($items as $item) {
                        ...
                        $order->status_id = 1;
                        $order->operator_id = $this->distribute($site->showroom_id);
                        $order->type_id = 21;
                        ...
                        try {
                            ...
                            Log::error("type error Avangard vdl", [$item]);
                        }
