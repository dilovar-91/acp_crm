...
class ImportAutodomController extends Controller
{
    public function import()
    {
        ...
        $sites = Site::whereIn('id', [6668, 6212])->whereNotNull('token')->with(['request'])->get();

        // Log::error("autodom sites", [$sites]);
        ...
        if (count($sites) > 0) {
            foreach ($sites as $site) {
                ...
                try {
                    ...
                    // Log the exception
                    Log::error("Request Exception autdom: " . $e->getMessage());
                }
                ...
                if (is_array($items) && count($items) > 0) {
                    ...
                    foreach ($items as $item) {
                        ...
                        $order->status_id = 1;
                        $order->operator_id = $this->distribute(10);
                        $order->type_id = 21;
                        ...
                        try {
                            ...
                            Log::error("type error autodom vdl", [$item]);
                        }
