...
class ImportAutopoleController extends Controller
{
    ...
    {
        $sites = Site::whereIn('id', [6692])->whereNotNull('token')->with(['request'])->get();
        if (count($sites) > 0) {
            foreach ($sites as $site) {
                ...
                try {
                    ...
                    // Log the exception
                    Log::error("Request Exception autoploe pv: " . $e->getMessage());
                }
                ...
                if (is_array($items) && count($items) > 0) {
                    foreach ($items as $item) {
                        ...
                        try {
                            ...
                            Log::error("type error autoploe vdl", [$item]);
                        }
