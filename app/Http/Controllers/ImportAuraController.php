...
class ImportAuraController extends Controller
{
    ...
    {
        $sites = Site::whereIn('id', [6680, 6028])->whereNotNull('token')->with(['request'])->get();
        if (count($sites) > 0) {
            foreach ($sites as $site) {
                ...
                try {
                    ...
                    // Log the exception
                    Log::error("Request Exception aura pv: " . $e->getMessage());
                }
                ...
                if (is_array($items) && count($items) > 0) {
                    foreach ($items as $item) {
                        ...
                        $order->status_id = 1;
                        $order->operator_id = $this->distribute(7);
                        $order->type_id = 21;
                        ...
                        try {
                            ...
                            Log::error("type error aura vdl", [$item]);
                        }
