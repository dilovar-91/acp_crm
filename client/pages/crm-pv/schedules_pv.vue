        ...
        <template #top>
          ...
          >
            <v-toolbar-title class="mr-3">
              График работы операторов {{showroom?.name}}
            </v-toolbar-title>
            <v-btn color="primary" class="mb-1 mr-2" @click="dialog=true">
              <v-icon>mdi-plus</v-icon>
            </v-btn>
            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/2/schedules" >
              АМК
            </v-btn>

            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/4/schedules" >
              Автопремиум
            </v-btn>

            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/5/schedules" >
              Автомаксимум
            </v-btn>
            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/7/schedules" >
              Форсаж
            </v-btn>

            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/8/schedules" >
              Автопремьер
            </v-btn>

            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/10/schedules" >
              Автодом
            </v-btn>

            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/13/schedules" >
              Автоплюс
            </v-btn>
            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/14/schedules" >
              Автопорт
            </v-btn>
            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/15/schedules" >
              Авангард
            </v-btn>
            <v-btn color="primary" class="mb-1 mr-2" to="/crm-pv/17/schedules" >
              Автополе
            </v-btn>
            <v-spacer/>
            <v-dialog
...
export default {
  ...
  async fetch({store, params: {id}}) {
    ...
    await store.dispatch('operator/fetchProgrammaticSchedules', {showroom_id: id})
  ...
  computed: {
    ...
    schedules() {
      return this.$store.state.operator.programmatic_schedules
    },
    showroom() {
  ...
  methods: {
    ...
    async deleteItemConfirm() {
      try {
        if (this.editedIndex > -1) {
          await this.$store.dispatch('operator/deleteProgrammaticSchedule', {
            id: this.editedIndex,
    ...
    async save() {
      ...
      try {
        if (this.editedIndex > -1) {
          await this.$store.dispatch('operator/updateProgrammaticSchedule', {item})
          ...
          await this.$store.dispatch('operator/createProgrammaticSchedule', {item})
