<template>
  <v-container fluid>
    ...
    <v-row justify="center">
      <v-col cols="12" xl="10" md="12" lg="11" class="pt-0">
        ...
        <v-toolbar flat dense class="mt-0 mb-0">
          ...
          <v-toolbar-title>Изменение заявки (№<span class="font-weight-bold">{{ form?.id }}</span>)
          </v-toolbar-title>
          <v-spacer/>
          <v-btn color="blue" class="mr-2 white--text" :to="'/crm-pv/' + $route.params?.showroom + '/orders'">
              ...
              >
                <v-row dense>
                  ...
                  <v-col cols="12" md="12" xl="12" sm="12">
                    <v-row dense>
                      <v-col cols="12" sm="12" xl="4" md="4" class="pa-0 mr-10">
                        Последный прозвон
                        <DTPicker
                      ...
                      <v-col cols="12" sm="12" xl="4" md="4" class="py-0 mr-10">
                        Перезвонить
                        <DTPicker
                      ...
                      <v-col cols="12" sm="12" xl="4" md="4" class="mt-4" v-show="form.status_id === 7">
                        <v-select
                      ...
                      <v-col cols="12" sm="12" xl="4" md="4" class="mt-4">
                        <v-select
                          ...
                          :items="operators.filter(l=>l.showroom_id == $route.params.showroom)"
                          item-value="id"
                          ...
                          required
                          hide-details
                          :label="'Оператор ' + $route.params.showroom"
                          :rules="[validateOperator]"
                          :disabled="this.role_id === 2 && this.form.operator_id !== null"
                        >
                        </v-select>
                      </v-col>
                      <v-col cols="12" sm="12" xl="4" md="4" class="mt-4">
                        <v-select
                          v-model="passOperator"
                          :items="operators.filter(l=>l.showroom_id == $route.params.showroom)"
                          item-value="id"
                          :item-text="item => item.last_name ? item.first_name + ' ' + item.last_name + '(' + item?.work_place + ')' : item.first_name + '(' + item?.work_place + ')'"
                          dense
                ...
                ><br/>
                  ...
                  <v-col cols="12" xl="12" sm="12" md="12">
                    ...
                    <v-row class="mb-2" dense>
                      ...
                      <v-col cols="12" md="6">
                        <v-btn width="170px" color="success" dark @click="distributeOpen(7)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          Форсаж
                        </v-btn>
                      </v-col>
                      <v-col cols="12" md="6">
                        <v-btn width="170px" color="purple" dark @click="distributeOpen(10)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          Автодом
                        </v-btn>
                      </v-col>
                      ...
                      <v-col cols="12" md="6">
                        <v-btn width="170px" color="warning" :loading="busy" dark @click="distributeOpen(2)">
                          <v-icon>mdi-arrow-right</v-icon>
                          АМК
                        </v-btn>
                      </v-col>
                      <v-col cols="12" md="6">
                        <v-btn width="170px" color="red" dark @click="distributeOpen(4)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          АвтоПремиум
                        </v-btn>
                      </v-col>
                      <v-col cols="12" md="6">
                        <v-btn width="170px" color="green" dark @click="distributeOpen(5)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          Авангард Юг
                        </v-btn>
                      </v-col>
                      ...
                      <v-col cols="12" md="6">
                        <v-btn width="170px" color="yellow" @click="distributeOpen(8)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          АвтоПремьер
                        </v-btn>
                      </v-col>
                      ...
                      <v-col cols="12" md="6">
                        <v-btn width="170px" color="lime" @click="distributeOpen(13)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          АвтоПлюс
                        </v-btn>
                      </v-col>
                      <v-col cols="12" md="6">
                        <v-btn width="170px" class="orange darken-3" dark @click="distributeOpen(14)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          Автопорт
                        </v-btn>
                      </v-col>
                      <v-col cols="12" md="6">
                        <v-btn width="100%" class="blue-grey darken-3" dark @click="distributeOpen(15)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          Авангард
                        </v-btn>
                      </v-col>
                      ...
                      <v-col cols="12" md="6">
                        <v-btn width="100%" class="brown darken-2" dark @click="distributeOpen(17)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          Автополе
                        </v-btn>
                      </v-col>
    ...
    <v-row justify="center">
      <v-col cols="12" xl="10" md="12" lg="11" class="pt-0">
        ...
        >
          ...
          <v-icon v-if="history_active" color="primary" right>mdi-chevron-up</v-icon>
                      ...
                      <template v-if="item.recording_id && listen_record.includes(role_id) ">
                        ...
                        <div class="d-flex align-center justify-center mb-2">
                          <v-sheet class="ma-2 pa-2">
                            <audio :src="'/api/record2/' + showroom_id + '/' + item.recording_id" controls
        ...
        <v-dialog v-model="isOpenDistribute" max-width="400px">
          <v-card>
            <v-card-title>
              <span class="headline">Передача заявки {{ showrooms.find(l => l.id === to_showroom)?.name }}</span>
            </v-card-title>
            <v-card-text>
              <p>Вы уверены что передать эту заявку в {{ showrooms.find(l => l.id === to_showroom)?.name }}?</p>

              <v-col cols="12" class="py-0 text-right">
                <v-select
                  v-model="passOperator"
                  :items="operators.filter(l=>l.showroom_id === to_showroom)"

                  item-value="id"
                  :item-text="item => item.last_name ? item.first_name + ' ' + item.last_name + '(' + item?.work_place + ')' : item.first_name"
                  dense
                  outlined
                  required
                  hide-details
            ...
            <v-card-actions>
              <v-spacer/>
              <v-btn :disabled="passOperator === null" color="success" @click="distribute()">Да</v-btn>
    ...
    <repeat-dialog :show.sync="showRepeatDialog"
...
export default {
  ...
  async fetch({store, params: {id, showroom}}) {
  ...
  computed: {
    ...
    showroom_id() {
      return parseInt(this.$route.params?.showroom)
    },
    showroom() {
      return this.showrooms.find(l => l.id === this.showroom_id)
    },
    drops() {
    ...
    links() {
      return [
        ...
        },
        {
          text: 'CRM',
          disabled: false,
          href: '/crm-pv/' + this.showroom_id
        },
        {
          text: 'Заявки',
          disabled: false,
          href: '/crm-pv/' + this.showroom_id + '/orders'
        },
        {
          text: 'Изменение Programmatic V ' + this.showroom?.name,
          disabled: true,
          href: '/crm-pv/' + this.showroom_id + '/orders'
        },
      ]
  ...
  created() {
    this.$axios.get('/order/' + this.showroomPairs[this.$route.params?.showroom] + '/detail/' + this.$route.params?.id)
      .then(
        async response => {
  ...
  methods: {
    async save(after) {
      try {
        ...
        if (after === "new") {
          ...
          this.$router.push('/crm-pv/' + this.showroom_id + '/orders');
        }
        if (this.form.last_call !== null) {
          this.form.last_call = this.$moment(this.form.last_call).format('DD.MM.YYYY HH:mm');
        }
    ...
    async call(phone) {
      ...
      this.apiForm.showroom_id = this.showroom_id;
    ...
    async copyOrder() {
      const item = {
        ...
        order_id: this.showroom_id,
        site_id: this.form.site_id,
        type_id: 21,
      };
      if (this.$refs.copy_form.validate() && this.copyValid) {
        try {
          await this.$store.dispatch('order/copyOrder', {item}).then((res) => {
    ...
    async distribute() {
      const item = {
        showroom_id: this.to_showroom ?? this.$route.params?.showroom,
        order_id: this.$route.params?.id,
        operator_id: this.passOperator,
        site_id: this.form?.site_id,
        type_id: 21,
      };
      if (item.showroom_id !== null) {
        try {
