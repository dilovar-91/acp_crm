    ...
    <v-container fluid class="pt-0">
      <v-row no-gutters align="start" class="d-flex">
          ...
          <v-card class="mx-auto pt-0">
              ...
              <v-row class="ml-3 1">
                <v-col cols="12" sm="6" md="10">
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" xl="2" md="3" class="hidden-sm-and-down mt-1">
                      <v-text-field
                        v-model="search"
                        clearable
                        label="Поиск"
            ...
            <v-card-text class="pa-0 py-0">
              <template>
                ...
                <v-btn
                  :color="(filter_status==14 ? 'green darken-1' : 'darken-1')"
                  text
                  @click="changeStatus(14)"
                >
                  Корзина А/О
                </v-btn>
              ...
              >
                ...
                >
                  ...
                  >
                    <td>
                      <nuxt-link :to="'/crm-pv/' + showroom_id + '/order-edit/'+item.id"
                                 :class="row_classes(item)">
                    ...
                    <td>
                      <a v-if="$hasDomain(item.utm_source)" target="_blank" rel="noreferrer"
                         :href="'https://' + item.utm_source">{{ performPunyDecode(item.utm_source) }}

                      </a>
                      <span v-else>{{ performPunyDecode(item.utm_source) }}</span>
                    </td>
            ...
            <template>
              <v-simple-table style="background-color: #fdfdfd;">
                <template v-slot:default>
                  ...
                  >
                    <td>
                      <nuxt-link :to="'/crm/'+showroom?.id + '/order-pv/'+item.id + '/edit-mini'">
                        {{ item.id }}
                      </nuxt-link>
                    </td>
                    <td>{{ item.phone }}</td>
        ...
        <v-dialog v-model="isOpenDistribute" max-width="400px">
          <v-card>
            <v-card-title>
              <span class="headline">Передача заявки в {{ showroom?.name }}</span>
            </v-card-title>
            <v-card-text>
              <v-container>
                <v-form
...
export default {
  ...
  computed: {
    ...
    links() {
      return [
        ...
        {
          text: "Programmatic V " + this.showroom?.name,
          disabled: true,
          href: '/'
        }
      ]
  ...
  watch: {
    ...
    isOpenDistribute(val) {
      val || this.closeDistribute()
    },
    repeatDialog(val) {
      val || (this.repeats = [])
    },
  ...
  created() {
    if (this.$route.query.from) {
      ...
      this.filter_from = this.$moment().subtract(1, 'days').format('YYYY-MM-DD') + ' 18:30'
    }
    if (this.$route.query.to) {
      this.filter_to = this.$route.query.to
    } else {
      this.filter_to = this.$moment().format('YYYY-MM-DD') + ' 18:30'
    }
  ...
  methods: {
    ...
    performPunyDecode(domain) {
      try {
        // Add a default scheme if it's missing (assumes 'https://')
        ...
        const decodedHostname = hostnameParts.map(part => {
          // Decode Punycode parts of the domain
          return part.startsWith('xn--') ? punycode.decode(part.slice(4)) : part;
        }).join('.');

        // Rebuild the URL with the decoded hostname
        return `${urlObj.protocol}//${decodedHostname}${urlObj.pathname}${urlObj.search}${urlObj.hash}`;
      } catch (error) {
        console.error('Failed to decode URL:', error);
        return domain;  // Return the original domain in case of failure
      }
    },
    async refresh_page() {
        ...
        setTimeout(
          async () => {
            await this.$store.dispatch('order/fetchLightOrders', {
              id: this.showroomPairs[this.$route.params?.id],
    ...
    async openRepeat(item) {
      ...
      this.repeatItem.showroom_id = this.showroomPairs[this.$route.params?.id]
        ...
        await this.$store
          ...
          .then(async (res) => {
            const item = {
              ...
              operator_id: this.passOperator ?? null,
              type_id: 21,
              isVictory: true,
            };
            if (this.$refs.copy_form.validate() && this.copyValid) {
              try {
                await this.$store.dispatch('order/distributeOrder', {item}).then((res) => {
