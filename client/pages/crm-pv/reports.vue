...
import ByOperatorVictory from '~/components/report/ByOperatorVictory'
...
export default {
  ...
  created() {
    ...
    const sixTime = ' 18:30:00'
    this.date_from = this.$moment().subtract(1, 'days').format('DD.MM.YYYY') + sixTime
    this.date_to = this.$moment().format('DD.MM.YYYY') + sixTime
  },
  ...
  methods: {
    ...
    fillDate(interval) {
      ...
      switch (interval) {
        ...
        case 'six':
          ...
          this.date_to = this.$moment().format('DD.MM.YYYY') + six_time
          return
        case 'yesterday':
    ...
    async fetchReport() {
      ...
      try {
        ...
        const endpoint = endpointMap[this.type_id] || 'https://accas.ru/api/report/operator/victory';
    ...
    getData() {
      return {
        ...
        showroom_id: this.showroomPairs[this.$route.params?.id],
