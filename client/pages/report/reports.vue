<template>
  <div>
    <v-card class="mx-auto">
      ...
      <div class="px-2 d-flex flex-row">
        ...
        <v-sheet class="px-2">
          <v-select
            ...
            hide-details
            clearable
            @click:clear="$nextTick(() => (showroom_id = null))"
          />
        </v-sheet>
        <v-sheet class="px-2">
          <v-select
            ...
            hide-details
            clearable
            @click:clear="$nextTick(() => (agency_id = null))"
          />
        </v-sheet>
        <v-sheet class="px-2">
          <date-picker
      ...
      />
      <SummaryApprovals
        v-if="type_id === 4 && reportData.length"
        :date_from="date_from"
        :date_to="date_to"
        :data="reportData"
      />
      <SummaryDrops
        v-if="type_id === 5 && reportData.length"
        :date_from="date_from"
        :date_to="date_to"
        :data="reportData"
      />
      <SummaryVictory
        v-if="type_id === 6 && reportData.length"
        :date_from="date_from"
        :date_to="date_to"
        :data="reportData"
      />
      <SummaryOperator
        v-if="type_id === 7 && reportData.length"
        :date_from="date_from"
        :date_to="date_to"
        :data="reportData"
      />
      ...
      <SummaryBySites
        v-if="type_id === 8 && reportData.length"
        :date_from="date_from"
        :date_to="date_to"
        :data="reportData"
      />
...
import SummaryArrivals from '~/components/report/SummaryArrivals'
import SummaryApprovals from '~/components/report/SummaryApprovals'
import SummaryDrops from '~/components/report/SummaryDrops'
import SummaryVictory from '~/components/report/SummaryVictory'
import SummaryOperator from '~/components/report/SummaryOperator'
import SummaryBySites from '~/components/report/SummaryBySites.vue'
...
export default {
  ...
  methods: {
    ...
    fillDate(interval) {
      this.report = false
      const before_time = ' 00:00:00'
      const after_time = ' 23:59:59'
      const seven_time = ' 19:00:00'
      switch (interval) {
        case 'today':
        ...
        case 'tomorrow':
          this.date_from =
            this.$moment().add(1, 'days').format('DD.MM.YYYY') + before_time
          this.date_to =
            this.$moment().add(1, 'days').format('DD.MM.YYYY') + after_time
          return
        case 'week':
    ...
    async fetchReport() {
      ...
      const endpointMap = {
        2: 'reports/arrivals',
        4: `${process.env.API_URL}/reports/operator`,
        5: `${process.env.API_URL}/reports/drops`,
        6: `${process.env.API_URL}/reports/victory`,
        7: `${process.env.API_URL}/reports/by-operator`,
        8: `${process.env.API_URL}/reports/agency-sites`,
      }
      try {
        const query = this.getData()
        const endpoint =
          endpointMap[this.type_id] || `${process.env.API_URL}/reports/all`
