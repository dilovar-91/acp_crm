      ...
      <v-row no-gutters align="start" class="d-flex">
          ...
          <v-card class="mx-auto pt-0">
            <v-app-bar class="grey lighten-5" elevate-on-scroll dense>
              ...
              <v-btn
                ...
                dark
                class="mr-2"
                :to="'/tablet/credit-request/' + showroom_id"
              >
                  ...
                  <tbody>
                    <template v-for="(item, i) in arrivals">
                      ...
                      >
                        ...
                        <td>{{ item.operator && item.operator.first_name }}</td>
                        <td :class="siteClass(item.site)">
                          {{ item.site && item.site.title }}
                        </td>
                        <td v-html="setLinks(item.comments)"></td>
                        <td v-html="setLinks(item.sd_comment)"></td>
                        <td>
                          <v-chip
        ...
        <v-dialog v-model="dialog" max-width="900">
          <v-card>
            ...
            <v-card-actions>
              <v-btn
                v-if="editedIndex !== -1 && editedItem?.payment_method !== 1"
                color="warning darken-1"
                class="mr-3"
                text
                @click="toCredit(editedItem.id)"
              >
                Передать в крединый отдел
              </v-btn>
              <v-spacer />
              <v-btn
                v-if="editedIndex !== -1"
...
export default {
  ...
  computed: {
    ...
    visit_count() {
      return this.$store.state.arrival.arrivals.filter((l) => l.visited === 1)
    },
    not_coming_count() {
      return this.$store.state.arrival.arrivals.filter(
        (l) => l.not_coming === 1
      )
    },
  ...
  methods: {
    ...
    deleteItem(id) {
      this.$store
        .dispatch('arrival/deleteArrival', {
          id: this.deleteId,
        ...
        this.$store
          ...
          .then((res) => {
            if (this.editedIndex > -1) {
              this.$notify('success', 'Приезд создан')
            } else {
              this.$notify('success', 'Приезд добавлён')
            }
            this.close()
          })
          .catch((error) => {
            this.$notify('error', 'Заполните обязательные поля' + error)
          })
      } else {
    ...
    row_classes(item) {
      ...
      } else if (item.not_responding === 1) {
        return 'amber darken-2 white--text'
      } else if (item.consultation === 1) {
    ...
    exportFile() {
      ...
      const rows = this.arrivals.map((row, index) => ({
        ...
        'Источник рекламы': row.ads_source,
      }))
