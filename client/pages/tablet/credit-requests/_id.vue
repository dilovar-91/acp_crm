  ...
  <div>
    ...
    <v-container fluid>
      <v-row no-gutters align="start" class="d-flex">
                ...
                <v-card>
                  <v-list>
                    ...
                    <v-list-item>
                      <v-list-item-action>
                        <v-switch
                          v-show="$gates.hasRole('admin')"
                          test="
                            this.role &&
                              (this.role?.name === 'admin' || this.role?.name === 'mark' || this.role?.name === 'vova' ||
                    ...
                    <template v-for="(item, i) in items">
                      ...
                      >
                        ...
                        <td>
                          ...
                          {{ item.manager?.last_name }}<br />
                          {{ item.manager2?.first_name }}
                          {{ item.manager2?.last_name }}
                        </td>
                        <td>
                          {{ item.client_name }}
                          <template v-if="item.parent !== null">
                            <p>({{ item.parent?.client_name }})</p>
                          </template>
                        </td>
                        <td>
          ...
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn
                ...
                class="mr-3"
                text
                @click="openCoApplicantDialog(editedItem.id)"
              >
                Добавить созаёмщика
              </v-btn>
              ...
              <v-btn
                v-if="editedItem.deleted_at !== null && editedIndex !== -1"
                color="red darken-1"
                class="mr-3"
                text
              ...
              <v-btn
                ...
                class="mr-3"
                text
                @click="confirmRestore(editedItem.id)"
              >
                Восстановить
              </v-btn>
              <v-btn
                ...
                text
                :disabled="
                  editedItem.deleted_at !== null || canDelete() === false
                "
        ...
        <v-dialog v-model="dialogRestore" max-width="600">
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn color="green darken-1" text @click="restoreItem()">
                Да
              </v-btn>
              <v-btn color="green darken-1" text @click="dialogRestore = false">
                Нет
              </v-btn>
            </v-card-actions>
          </v-card>
        ...
        <v-dialog v-model="applicantDialog" max-width="600">
          <v-card>
            <v-card-title> Заполните форму </v-card-title>

            <v-row class="mx-4">
              <v-col cols="12" sm="12" md="12">
                <v-text-field
                  class="mt-4"
                  v-model="appItem.name"
                  label="ФИО созаёщика"
                  outlined
                  dense
                  hide-details
            ...
            <v-card-actions>
              ...
              <v-btn color="green darken-1" text @click="addCoApplicant()">
                Да
              </v-btn>
              <v-btn
                color="green darken-1"
                text
                @click="applicantDialog = false"
              >
                Нет
              </v-btn>
            </v-card-actions>
...
export default {
  ...
  data: (vm) => ({
    ...
    nameRules: [
      ...
      (v) =>
        (v && v.length >= 6) || 'ФИО клиента должно быть более 10 символов',
    ],
    headers: [
      {
        text: '№',
  ...
  async fetch({ store, params: { id } }) {
    ...
    await store.dispatch('showroom/fetchSecondManagers', { showroom_id: id })
  ...
  computed: {
    ...
    managers2() {
      return this.$store.state.showroom.managers2
    },
    preparers() {
  ...
  methods: {
    ...
    openCoApplicantDialog(id) {
      this.appItem.id = id
      this.applicantDialog = true
    },
    async addCoApplicant() {
      await this.$axios
        .post('/credit-requests/sync', {
          id: this.appItem.id,
          name: this.appItem.name,
        })
        .then(({ data }) => {
          if (data.id) {
            this.$toast.success('Созаёмщик добавлен!', {
              position: 'top-right',
              timeout: 2000,
              closeOnClick: true,
          ...
          this.appItem.id = null
          this.appItem.name = null
        })
        .catch((error) => {
    ...
    deleteItem() {
      if (this.fromTrash) {
        this.$store
          .dispatch('credit/deleteTrashCreditRequest', {
            id: this.saleItem && this.saleItem.id,
          })
          .then((res) => {
            this.$toast.success('Приезд удалён из корзины')
          })
          .catch((error) => {
    ...
    restoreItem() {
      this.$store
        .dispatch('credit/restoreCreditRequest', {
          id: this.editedItem?.id,
          showroom_id: this.showroom_id,
        })
        .then((res) => {
          this.$toast.success('Приезд восстановлен')
        })
        .catch((error) => {
          this.$toast.error('Ошибка: ' + error)
        })
    ...
    saledItem() {
      this.$store
        ...
        .then((res) => {
          console.log(this.saleItem)
          this.$toast.success('Запись изменён')
        })
        .catch((error) => {
          this.$toast.error('Ошибка: ' + error)
        })
    ...
    getManager(id) {
      const man = this.managers2.find((l) => l.id === id)?.first_name
      if (man !== undefined) {
        return '(' + man + ')'
      } else {
        return ''
    ...
    row_classes(item) {
      switch (true) {
        case item.deleted_at !== null:
          return 'black white--text'
        case item.canceled === 1:
          return 'red white--text'
        case item.client_canceled === 1:
          return 'grey lighten-1'
        case item.fssp_canceled === 1:
    ...
    approvedNotify() {
      ...
      if (banksToCheck.some((bank) => banks[bank] === 4 || banks[bank] === 5)) {
