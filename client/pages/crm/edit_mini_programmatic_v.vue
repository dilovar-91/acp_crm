    ...
    <v-row justify="center">
      <v-col cols="12" xl="10" md="12" lg="11" class="pt-0">
        ...
        <v-toolbar flat dense class="mt-0 mb-0">
          ...
          <v-spacer/>
          <v-btn color="blue" class="mr-2 white--text" :to="'/crm/7/orders-pv'">
                ...
                ><br/>
                  ...
                  <v-col cols="12" xl="12" sm="12" md="12">
                    ...
                    <v-row class="mb-2" dense>
                      <v-col cols="12" md="12">
                        <v-btn width="100%" color="success" dark @click="distributeOpen(7)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          Форсаж-Авто
                        </v-btn>
                      </v-col>
                      <v-col cols="12" md="6">
                        <v-btn width="100%" color="purple" dark @click="distributeOpen(10)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          Автодом
                        </v-btn>
                      </v-col>
                      ...
                      <v-col cols="12" md="6">
                        <v-btn width="170px" class="blue-grey darken-3"  dark @click="distributeOpen(15)" :loading="busy">
                          <v-icon>mdi-arrow-right</v-icon>
                          Авангард
                        </v-btn>
                      </v-col>
...
export default {
  ...
  async fetch({store}) {
    ...
    await store.dispatch('showroom/fetchSites', {id: (7 || this.form?.showroom_id)})
    await store.dispatch('credit/fetchBanks')
    await store.dispatch('showroom/fetchManagers', {showroom_id: (7 || this.form?.showroom_id)})
    await store.dispatch('showroom/fetchOperators', {showroom_id: (7 || this.form?.showroom_id), orderWorkers: false})
    ...
    links() {
      return [
        ...
        },
        {
          text: 'Заявки',
          disabled: false,
          href: '/crm/7/orders-pv'
        },
        {
          text: 'Изменение Programmatic V В',
          disabled: true,
  ...
  created() {
    this.$axios.get('/order/19/detail/' + this.$route.params?.id)
      .then(
        async response => {
    ...
    async save(after) {
      try {
        ...
        if (after === "new") {
          ...
          this.$router.push('/crm/7/orders-pv');
        }
        if (this.form.last_call !== null) {
          this.form.last_call = this.$moment(this.form.last_call).format('DD.MM.YYYY HH:mm');
        }
