            ...
            <v-card-text class="pa-0 py-0">
              ...
              >
                ...
                >
                  ...
                  >
                    <td>
                      <nuxt-link :to="'/crm/7/order-edit/'+item.id"
                                 :class="row_classes(item)">
            ...
            <template>
              <v-simple-table style="background-color: #fdfdfd;">
                <template v-slot:default>
                  ...
                  >
                    <td>
                      <nuxt-link :to="'/crm/7/order-pv/'+item.id + '/edit-mini'">
                        {{ item.id }}
                      </nuxt-link>
                    </td>
                    <td>{{ item.phone }}</td>
...
export default {
  ...
  async fetch({store, $auth}) {
    ...
    await store.dispatch('showroom/fetchShowroom', {id: 7})
    await store.dispatch('showroom/fetchShowrooms')
    await store.dispatch('showroom/fetchSites', {id: 7})
    await store.dispatch('showroom/fetchManagers', {id: 7})
    await store.dispatch('showroom/fetchOperators', {showroom_id: 7, orderWorkers: false})
    await store.dispatch('order/fetch_arrivals', {id: 7})
    await store.dispatch('order/fetch_missed_calls', {id: 7})
  ...
  methods: {
    ...
    async refresh_page() {
      ...
      if (this.lastUpdate && this.$moment().diff(this.lastUpdate, 'seconds') <= 10) {
        ...
        setTimeout(
          async () => {
            await this.$store.dispatch('order/fetchLightOrders', {id: 19, query: this.$route.query});
