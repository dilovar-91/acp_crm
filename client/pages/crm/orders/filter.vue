                  ...
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" xl="2" md="2" class="hidden-sm-and-down mt-1">
                      <v-select
                        ...
                        dense
                        @click:clear="
                          $nextTick(() => (clearField('filter_site')))
                        "
                      >
                    ...
                    <v-col cols="12" sm="6" xl="2" md="2" class="hidden-sm-and-down mt-1">
                      <v-select
                        ...
                        dense
                        @click:clear="
                          $nextTick(() => (clearField('filter_type')))
                        "
                      >
                        <template #selection="{ item, index }">
                          <template v-if="index === 0">
                    ...
                    <v-col cols="12" sm="6" xl="2" md="2" class="hidden-sm-and-down mt-1">
                      <v-select
                        ...
                        dense
                        @click:clear="
                          $nextTick(() => (clearField('filter_status')))
                        "
                      >
                        <template #selection="{ item, index }">
                          <template v-if="index === 0">
...
export default {
  ...
  async fetch({store, params: {id}, $auth}) {
    ...
    await store.dispatch('order/fetchOrders2', {id, query: $nuxt.$route.query})
  ...
  created() {
    if (this.$route.query.from) {
      this.filter_from = this.$moment(this.$route.query.from).format('DD.MM.Y')
    }
    if (this.$route.query.to) {
      this.filter_to = this.$moment(this.$route.query.to).format('DD.MM.Y')
    }
  ...
  methods: {
    ...
    async changedPage() {
      ...
      await this.$store.dispatch('order/fetchOrders2', {id: this.$route.params.id, query: this.$route.query});
      ...
      try {
        ...
        await this.$router.push({
          query: {
            ...
            ...(this.filter_from && {from: this.$moment(this.filter_from, "DD.MM.YYYY").format('YYYY-MM-DD')}),
            ...(this.filter_to && {to: this.$moment(this.filter_to, "DD.MM.YYYY").format('YYYY-MM-DD')}),
            page: 1
          }
        });
      } catch (error) {
    ...
    async clearField(field) {
      const {query} = this.$route;
      try {
        await this.$router.replace({query: {...query, status: undefined}});
      } catch (error) {
        if (error.name !== 'NavigationDuplicated') {
    ...
    async clearFilter() {
      ...
      const {query} = this.$route;
      try {
        await this.$router.push({path: `/crm/${this.$route.params.id}/orders`,
          query: {}});
      } catch (error) {
        if (error.name !== 'NavigationDuplicated') {
          //
    ...
    async changeStatus(status) {
      if (this.status_id == status) return;

      const {query} = this.$route;
      try {
        await this.$router.replace({query: {...query, status, page: 1}});
      } catch (error) {
        if (error.name !== 'NavigationDuplicated') {
        ...
        await this.$store.dispatch('order/fetchOrders2', {id: this.$route.params.id, query});
        this.page = 1;
