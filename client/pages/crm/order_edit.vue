<template>
  <v-container fluid>
    ...
    <v-row justify="center">
      <v-col cols="12" xl="11" md="12" lg="11" class="pt-0">
        ...
        <v-toolbar flat dense class="mt-0 mb-0">
          ...
          <v-spacer/>
          <v-btn color="blue" class="mr-2 white--text" :to="'/crm/'+ form?.showroom_id+'/orders'">
        ...
        <v-form ref="form" v-model="valid">
          ...
          >
            <v-row dense>
              ...
              <v-btn
                target="_blank"
                :href="`https://api.whatsapp.com/send/?phone=${form.phone?.match(/\d/g)?.join('')}&text&type=phone_number&app_absent=0`"
                class="ma-2 mt-4"
                icon
              >
                <v-icon
          ...
          <v-row no-gutters class="bottom-border">
                  ...
                  <v-col cols="12" sm="12" xl="3" md="3" class="pt-1">
                    ...
                    >
                      ...
                      >
                        ...
                        <v-icon small class="ml-2 mt-1" color="primary" @click="openDialogSms()"
                      ...
                      <v-col cols="12" sm="12" xl="4" md="4" class="pt-1">
                        ...
                        >
                          <template
                            slot="append"
                          >
                            <v-icon small class="mt-1" color="primary" @click="call(form.work_phone)"
                            >mdi-phone
                            </v-icon
                            >

                            <v-icon small class="ml-2 mt-1" color="primary" @click="openDialogSms('work_phone')"
                            >mdi-email-outline
                            </v-icon>
                          </template>
                        </v-text-field>
                      ...
                      <v-col cols="12" sm="12" xl="4" md="4" class="pt-1">
                        ...
                        >
                          <template
                            ...
                            slot="append"
                          >
                            <v-icon class="mt-1" color="primary" small @click="call(form.phone_2)"
                            >mdi-phone
                            </v-icon
                            >
                            <v-icon class="ml-2 mt-1" small color="primary" @click="openDialogSms('phone_2')"
                            >mdi-email-outline
                            </v-icon>
                          </template>
                        </v-text-field>
                      ...
                      <v-col cols="12" sm="12" xl="4" md="4" class="pt-1">
                        ...
                        >
                          <template
                            ...
                            slot="append"
                          >
                            <v-icon class="mt-1" color="primary" @click="call(form.phone_3)"
                                    small>mdi-phone
                            </v-icon
                            >

                            <v-icon class="ml-2 mt-1" color="primary" @click="openDialogSms('phone_3')"
                                    small>mdi-email-outline
                            </v-icon>
                          </template>
                        </v-text-field>
                      </v-col>
              ...
              >
                <v-row dense>
                  <v-col cols="12" sm="12" xl="12" md="12">
                    ...
                    <v-row dense>
                      <v-col
                        cols="12" sm="12" xl="1" md="1" class="py-1
              d-flex flex-row  px-auto">
                        <a
                          href="https://www.avito.ru/evaluation/cars" class="mr-2"
                          target="_blank" rel="noreferrer">
                          <v-img
                            style="border: #4caf50 1px solid; border-radius: 6px; cursor: pointer;"
                        ...
                        <a
                          ...
                          target="_blank" rel="noreferrer">
                          <v-img
                            style="border: red 1px solid; border-radius: 6px; cursor: pointer;"
                      ...
                      <v-col cols="12" sm="12" xl="2" md="2" class="py-0 mt-2 pl-4">
                        <v-text-field
                          v-model="form.car_number" color="primary"
                          dense
                          outlined
                          label="Гос. номер" hide-details="auto"
            ...
            <v-col cols="12" xl="2" sm="12" md="2" class="bottom-border">
              <v-card class="pa-2 py-0" outlined tile lign-self="scratch">
                ...
                >Точка входа:
                   <a :href="$formatUrl(form.entry_point)" target="_blank" rel="noreferrer">
                ...
                <v-row dense>
                  ...
                  <v-col cols="12" sm="12" xl="8" md="8" class="py-0">
                    <v-textarea
                      ...
                      label="Комментарий"
                      :readonly="
                          user_id !== order?.operator_id && role_id !== 1 &&  role_id !== 3 &&  role_id !== 6
                        "
                    />
                  </v-col>
              ...
              <v-card class="px-3 py-1" outlined tile>
                ...
                <template v-if="form?.defer_purchase">
                  <v-chip
                    ...
                    color="warning"
                  >
                    Отложенная покупка ({{ $moment(form?.defer_purchase?.return_date).format('DD.MM.YYYY') }})
                  </v-chip>
                  <v-btn
                    v-if="role_id !== 2"
    ...
    closeOnContentClick: false
      ...
      <v-dialog v-model="arriveDialog" persistent max-width="700px" content-class="arrive_dialog">
        <v-card>
          <v-card-title>
            ...
            <v-container>
              ...
              >
                <v-row dense>
                  ...
                  <v-col cols="4" class="py-0 text-right"> {{ arrival.arrival_date }}
                  ...
                  <v-col cols="4" class="py-0 text-right">
                    <v-select
                      ...
                      :items="payment_methods.filter(l=>{ return l.id===1 || l.id===2})"
                      item-text="name"
                      no-data-text="Нету данных"
                      item-value="id"
                      menu-props="auto"
                      ...
                      dense
                      clearable
                      @click:clear="$nextTick(() => arrival.payment_method=null)"
                    />
                  </v-col>
          ...
          <v-card-actions class="mt-n4">
            ...
            <v-checkbox
              ...
              dense
            />
            <v-btn class="mr-3" color="error" @click="closeArrive()">Отмена</v-btn>
            <v-btn class="mr-4" color="primary" @click="toArrive()">Отправить</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
                ...
                <v-row dense>
                  <v-col cols="12" class="py-0 text-right">
                    <v-select
                      ...
                      :items="sorted_showrooms.filter(l=>l.id != $route.params.showroom)"
                      item-value="id"
                      item-text="name"
                      placeholder="Салон"
      ...
      <repeat-dialog
        ...
        :title="'Заявка № '+ repeatItem.id +': другие заявки по номеру: ' + order.phone" :data="repeats"/>
      <repeat-dialog
        :show.sync="showAllRepeatDialog"
        :title="'Заявка № '+ order.id +': другие заявки по номеру: ' + order.phone" :data="all_repeats"
        :show-showroom="true"/>
          ...
          >
            ...
            >
              ...
              <td v-if="item.description === '5' || item.description === '6'">
                <table
                  v-if="item.properties?.create_time" class="activities"
                  width="100%">
                  <thead>
                  <tr>
                  ...
                  <tbody>
                  <tr>
                    <td>{{ $moment($moment.unix(item.properties?.create_time)).format('HH:mm:ss') }}</td>
                    <td>{{ $moment($moment.unix(item.properties?.end_time)).format("HH:mm:ss") }}</td>
                  </tr>
                  <tr>
                    <td colspan="2">
                      <template v-if="item.recording_id && listen_record.includes(role_id) ">


                        <div class="d-flex align-center justify-center mb-2">
                          <v-sheet class="ma-2 pa-2">
                            <audio
                              :src="'/api/record2/' + showroom_id + '/' + item.recording_id" controls
                              controlsList="nodownload"/>
                          </v-sheet>
                          <v-sheet class="ma-2 pa-2">
                            <v-icon @click="downloadRecord('/record3/' + showroom_id + '/' + item.recording_id)">
                              mdi-download
                            </v-icon>
                          </v-sheet>
                        </div>
                  ...
                  <template v-for="(row, i) in (item.properties && item.properties.attributes)">
                    <tr>
                      ...
                      <td>
                        ...
                        <template v-if="item.properties.old?.hasOwnProperty(i)">
                          ...
                          <template v-else-if="i === 'trash_id'">
                            {{ trashes.find(l => l.id == item.properties.old?.i)?.name }}
                          </template>
                          <template v-else-if="i === 'drop_id'">
                            {{ drops.find(l => l.id == item.properties.old?.i)?.name }}
                          </template>
                      ...
                      <td>
                        ...
                        <template v-else-if="i === 'drop_id'">
                          {{ drops.find(l => l.id === row)?.name }} {{ row }}
                        </template>
                        <template v-else-if="i === 'showroom_id'">
...
export default {
  ...
  async fetch({store, $axios, $gates, $auth, params: {showroom, id}}) {
    ...
    await store.dispatch('order/fetch_sms_templates', {id, operator_id: $auth.user?.id})
  ...
  computed: {
    ...
    tradein_models() {
      return this.$store.state.property.all_models.filter(l => l.brand_id === this.form?.tradein_mark_id)
    },
    filtered_models() {
      return this.$store.state.property.all_models.filter(l => l.brand_id === this.form?.mark_id)
    },
    showrooms() {
    ...
    operators() {
      if (this.role_id === 2 && this.form.operator_id === null) {
        return this.$store.state.showroom.operators.filter(l => l.id === this.user_id)
    ...
    this.$axios.get('/order/' + this.$route.params?.showroom + '/detail/' + this.$route.params?.id)
      .then(
        async response => {
          ...
          await this.$axios.post("orders/repeats", {item: this.repeatItem}).then(
            async response2 => {
  ...
  methods: {
    ...
    async toConsultation() {
      ...
      const postData = {
        ...
        model: this.models.find(l => l.id === this.form?.model_id)?.name || null,
        region: this.regions.find(l => l.id === this.form?.region_id)?.name || null,
      };
      const token = `$2a$10$wZOqtQTDzWtiWkhy5IX.S.5cTsYaxMju5fuzQUy9YJtSE6Nsz/1Gu`;

      this.$axios.defaults.headers.common.Authorization = `Bearer ${token}`;
      this.$axios.post('https://accas888.com/api/create/consultation', postData)
        .then(response => {
    ...
    async toArrive() {
      ...
      this.$axios.post('https://accas888.com/api/create/arrive', this.arrival)
        .then(response => {
          // Handle successful response
    ...
    async openArrive() {
      ...
      const mark = this.marks.find(l => l.id === this.form?.mark_id)?.name || '';
      const model = this.models.find(l => l.id === this.form?.model_id)?.name || '';
      const complectation = this.form?.complectation !== null ? (' ' + this.form?.complectation) : ''
      this.arrival.car_name = mark + ' ' + model + complectation
      this.arrival.client_name = this.form?.client_name
      this.arrival.operator = this.operators.find(l => l.id === this.form?.operator_id)?.first_name || null;
      this.arrival.price = this.form?.price
      this.arrival.initial_fee = this.form?.initial_fee
      this.arrival.payment_method = this.form.payment_method === 1 ? 1 : (this.form.payment_method === 2 ? 2 : null)
      ...
      this.arrival.site_name = this.sites.find(l => l.id === this.form?.site_id)?.title || this.types.find(l => l.id === this.form?.type_id)?.name;
      this.arrival_date = this.form?.will_arrive || this.$moment().format('YYYY-MM-DD')
    ...
    async save(after) {
      try {
        ...
        if (!statuses.includes(this.form?.status_id) && this.form?.region_id === null) {
          ...
          return;
        } else if (!lists.includes(this.form?.status_id) && this.form?.callback === null) {
        ...
        if (this.form.last_call !== null) {
          ...
          if (this.form.last_call !== this.oldLastCall && this.$moment(st, 'DD.MM.YYYY HH:mm:ss').isAfter()) {

            this.form.last_call = this.$moment(st, 'DD.MM.YYYY HH:mm:ss').format('DD.MM.YYYY HH:mm');
        ...
        if (this.form.callback !== null) {
          ...
          const midnight = this.$moment(call_back).startOf('day').format('YYYY-MM-DD HH:mm:ss');


          if (call_back === midnight) {
            this.$toast.error("Поле перезвонить не является корректной!!!!!");
            this.form.callback = this.$moment(cb, 'DD.MM.YYYY HH:mm:ss').format('DD.MM.YYYY HH:mm');
            return;
          }
          if (this.form.callback !== this.oldCallback && (this.$moment(cb, 'DD.MM.YYYY HH:mm:ss').isBefore() || this.$moment(cb, 'DD.MM.YYYY HH:mm:ss').isAfter(after3Months))) {
            this.$toast.error("Поле перезвонить не является корректной!!!");
            this.form.callback = this.$moment(cb, 'DD.MM.YYYY HH:mm:ss').format('DD.MM.YYYY HH:mm');
            this.form.last_call = this.$moment(this.form.last_call).format('DD.MM.YYYY HH:mm');
            return;
        ...
        if (this.form.will_arrive !== null) {
          ...
          if (this.form.will_arrive !== this.oldWillArrive && !this.$moment(this.form.will_arrive).isSameOrAfter(today, 'day')) {
        ...
        if (this.form.arrived_date !== null) {
          console.log(this.$moment(this.form.arrived_date).isSame(new Date(), "day"))
          if (this.form.arrived_date !== this.oldArrivedDate && !this.$moment(this.form.arrived_date).isSame(new Date(), "day") && this.role_id === 2) {
        ...
        if (this.form.last_call !== null) {
          this.form.last_call = this.$moment(this.form.last_call + ':00').format('DD.MM.YYYY HH:mm');
        }
        if (this.form.callback !== null) {
          this.form.callback = this.$moment(this.form.callback).format('DD.MM.YYYY HH:mm');
        }
        ...
        if (this.form.callback !== null) {
          this.form.callback = this.$moment(this.form.callback + ':00').format('DD.MM.YYYY HH:mm');
        }
    ...
    makeLink() {
      ...
      return `https://fssp.gov.ru/iss/ip?is[variant]=1&is%5Bregion_id%5D%5B0%5D=-1&${fio.length ? ('is%5Blast_name%5D=' + fio[0]) : ''}${fio.length > 0 ? ('&is%5Bfirst_name%5D=' + fio[1]) : ''}${fio.length > 1 ? ('&is%5Bpatronymic%5D=' + fio[2]) : ''}${birthday || ''}`
    },
    async call(phone) {
    ...
    copyName() {
      navigator.clipboard.writeText(this.form?.client_name).then(() => {
    ...
    calc(type, bank) {
      ...
      const result = (this.form.price - this.form.initial_fee) * (percent + (percent / (((1 + percent) ** period) - 1)));
      switch (type) {
        case 'payment':
          if (result <= 0) return
    ...
    async toDeferPurchase() {
      ...
      if (this.deferred_purchase_date !== null) {
        try {
          this.$store.dispatch('order/deferPurchase', {item}).then((res) => {
    ...
    async deleteDefer() {
      const confirmed = confirm("Вы действительно хотите удалить из отложенных?");
      ...
      try {
        ...
        this.$toast.error("Произошла ошибка при удалении заявки. Попробуйте позже.");
      }
    },
    async toBlacklist() {
      try {
        await this.$store.dispatch('order/blacklistRequest', {order_id: this.$route.params.id}).then((res) => {
    ...
    async openRepeats() {
      await this.$axios.post("orders/all-repeats", {item: this.order}).then(
        response => {
