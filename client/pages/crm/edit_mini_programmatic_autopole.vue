    ...
    <v-row justify="center">
      <v-col cols="12" xl="10" md="12" lg="11" class="pt-0">
        ...
        <v-toolbar flat dense class="mt-0 mb-0">
          ...
          <v-spacer/>
          <v-btn color="blue" class="mr-2 white--text" :to="'/crm/17/orders-pv'">
    ...
    <v-row justify="center">
      <v-col cols="12" xl="10" md="12" lg="11" class="pt-0">
        ...
        <v-dialog v-model="isOpenDistribute" max-width="400px">
          <v-card>
            <v-card-title>
              <span class="headline">Передача заявки {{ showrooms.find(l => l.id === this.to_showroom)?.name }}</span>
            </v-card-title>
            <v-card-text>
              <p>Вы уверены что передать эту заявку в {{ showrooms.find(l => l.id === this.to_showroom)?.name }}?</p>
...
export default {
  ...
  async fetch({store}) {
    ...
    await store.dispatch('showroom/fetchSites', {id: (17 || this.form?.showroom_id)})
    await store.dispatch('credit/fetchBanks')
    await store.dispatch('showroom/fetchManagers', {showroom_id: (17 || this.form?.showroom_id)})
    await store.dispatch('showroom/fetchOperators', {showroom_id: (17 || this.form?.showroom_id), orderWorkers: false})
  ...
  computed: {
    ...
    showroom() {
      return this.showrooms.find(l=>l.id === 17)
    },
    drops() {
  ...
  created() {
    this.$axios.get('/order/20/detail/' + this.$route.params?.id)
      .then(
        async response => {
    ...
    async save(after) {
      try {
        ...
        if (after === "new") {
          ...
          this.$router.push('/crm/17/orders-pv');
        }
        if (this.form.last_call !== null) {
          this.form.last_call = this.$moment(this.form.last_call).format('DD.MM.YYYY HH:mm');
        }
