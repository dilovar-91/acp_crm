            ...
            <v-card-text class="pa-0 py-0">
              ...
              >
                ...
                >
                  ...
                  >
                    <td>
                      <nuxt-link :to="'/crm/17/order-edit/'+item.id"
                                 :class="row_classes(item)">
                    ...
                    <td>
                      <a v-if="$hasDomain(item.utm_source)" target="_blank" rel="noreferrer" :href="'https://' + item.utm_source">{{ item.utm_source }}</a>
                      <span v-else>{{ item.utm_source }}</span>
                    </td>
            ...
            <template>
              <v-simple-table style="background-color: #fdfdfd;">
                <template v-slot:default>
                  ...
                  >
                    <td>
                      <nuxt-link :to="'/crm/17/order-pv/'+item.id + '/edit-mini'">
                        {{ item.id }}
                      </nuxt-link>
                    </td>
                    <td>{{ item.phone }}</td>
...
export default {
  ...
  async fetch({store, $auth}) {
    ...
    await store.dispatch('showroom/fetchShowroom', {id: 17})
    await store.dispatch('showroom/fetchShowrooms')
    await store.dispatch('showroom/fetchSites', {id: 17})
    await store.dispatch('showroom/fetchManagers', {id: 17})
    await store.dispatch('showroom/fetchOperators', {showroom_id: 17, orderWorkers: false})
    await store.dispatch('order/fetch_arrivals', {id: 17})
    await store.dispatch('order/fetch_missed_calls', {id: 17})
  ...
  methods: {
    ...
    async refresh_page() {
      ...
      if (this.lastUpdate && this.$moment().diff(this.lastUpdate, 'seconds') <= 10) {
        ...
        setTimeout(
          async () => {
            await this.$store.dispatch('order/fetchLightOrders', {id: 20, query: this.$route.query});
    ...
    async openRepeat(item) {
      ...
      this.repeatItem.showroom_id = this.showroom_id
