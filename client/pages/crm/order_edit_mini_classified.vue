    ...
    <v-row justify="center">
      ...
      <v-dialog v-model="isOpenDistribute" max-width="400px">
        <v-card>
          <v-card-title>
            <span class="headline"
              >Передача заявки
              {{ showrooms.find((l) => l.id === to_showroom)?.name }}</span
            >
          </v-card-title>
          <v-card-text>
            <p>
              Вы уверены что передать эту заявку в
              {{ showrooms.find((l) => l.id === to_showroom)?.name }}?
            </p>
          </v-card-text>
      ...
      >
        <v-card>
          <v-card-title>
            <span class="headline"
              >Передать завку в приезд ({{
                showrooms.find((l) => l.id === arrival.showroom_id)?.name
              }})</span
            >
          </v-card-title>
                ...
                <v-row dense>
                  <v-col cols="12" class="py-0 text-right">
                    <v-select
                      ...
                      :items="sorted_showrooms.filter((l) => l.id != 12)"
                      item-value="id"
                      item-text="name"
                      placeholder="Салон"
...
export default {
  ...
  async fetch({ store, $axios, $gates, $auth, params: { showroom, id } }) {
    ...
    await store.dispatch('showroom/fetchOperators', { showroom_id: 12 })
  ...
  created() {
    this.$axios
      .get('/order/12/detail/' + this.$route.params?.id)
      .then(async (response) => {
  ...
  methods: {
    ...
    async openArrive(showroom_id = null) {
      ...
      this.arrival.showroom_id = showroom_id
    ...
    async save(after) {
      try {
        ...
        if (after === 'new') {
          ...
          this.$router.push('/crm/' + this.form?.showroom_id + '/orders')
        }
        if (this.form.last_call !== null) {
          this.form.last_call = this.$moment(this.form.last_call).format(
            'DD.MM.YYYY HH:mm'
