          ...
          <v-card class="mx-auto pt-0">
            ...
            <v-card-text class="pa-0 py-0">
              <template v-if="!isSearch">
                ...
                <v-btn
                  :color="filter_status == 17 ? 'green darken-1' : 'darken-1'"
                  text
                  @click="setReturned()"
                >
                  Переданные
                  ...
                  <tbody>
                    ...
                    >
                      <td>
                        ...
                        <br />
                        <v-btn
                          v-if="item.defer_purchase?.returned !== 1"
                          color="primary"
                          dark
                          class="mb-2"
                          title="Возвращить в салон"
                          @click="returnDeferred(item)"
                        >
                      ...
                      <td>
                        ...
                        <p v-else style="color: orangered">Нет</p>
                      </td>
                      <td>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      {
        text: 'Дата возвращение ',
        align: 'center',
        sortable: false,
        width: '90px',
        value: 'order.deferred_return_date',
      },
      {
        text: 'Возвращен ',
        align: 'center',
  ...
  created() {
    ...
    this.$store.dispatch('order/fetchDeferredOrders', {
  ...
  methods: {
    ...
    refresh_page: _.debounce(async function () {
      ...
      if (
        ...
        setTimeout(async () => {
          await this.$store.dispatch('order/fetchDeferredOrders', {
      ...
      await this.$router
        .push({
          query: {
            ...
            ...(this.filter_showroom && { showroom_id: this.filter_showroom }),
            page: 1,
          },
        })
        .catch((err) => {
    ...
    async returnDeferred(item) {
      const result = window.confirm(
        'Вы действительно хотите вернуть отложенную заявку в салон?'
      )
      if (result) {
        try {
          await this.$store.dispatch('order/returnDeferPurchase', {
            order_id: item.id,
          })
          this.refresh_page()
        } catch (error) {
    ...
    async setReturned() {
      const { query } = this.$route
      this.filter_deferred_purchase = true
      this.page = 1
      try {
        await this.$router.push({
          query: { ...query, page: 1, returned: true },
        })
      } catch (error) {
        if (error.name !== 'NavigationDuplicated') {
        }
