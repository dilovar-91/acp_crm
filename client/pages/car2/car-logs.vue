          ...
          <v-card>
            ...
            >
                  ...
                  <tr v-for="(row, i) in (item.properties && item.properties.attributes)">
                    ...
                    <td>
                      <template v-for="(old_row, l) in item.properties.old">
                        <template v-if="l===i">
                          <template v-if="l === 'mark_id' || l === 'model_id' || l === 'showroom_id'">
                            {{ getValue(old_row, l) }}
                          </template>
                          <template v-else>
                            {{ old_row }}
                    ...
                    <td>
                      <template v-for="(row, k) in item.properties.attributes">
                        <template v-if="k===i">
                          <template v-if="k === 'mark_id' || k === 'model_id' || k==='showroom_id'">
                            {{ getValue(row, k) }}
                          </template>
                          <template v-else>
                            {{ row }}
              ...
              <template #item.properties.showroom_id="{ item }">
                <v-chip
v-if="item.properties && item.properties.showroom_id" class="ma-2" color="green"
                        text-color="white">
                  {{
...
export default {
  ...
  async fetch({store}) {
    await store.dispatch('activity/fetchCarActivities', {})
  ...
  methods: {
    async apply_filter() {
      await this.$store.dispatch('activity/fetchCarActivities', {
