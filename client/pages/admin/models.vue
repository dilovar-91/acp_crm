      ...
      <v-row no-gutters align="start" class="d-flex">
        <v-col cols="12" class="mt-1">
          <v-app-bar elevate-on-scroll dense>
            <v-toolbar-title>Список моделей</v-toolbar-title>
            <v-spacer/>
            ...
            <v-autocomplete
              ...
              label="Марка"
              outlined
              @change="getModels(mark_id)"
              @click="mark_id = null"
              @click:clear="
                            $nextTick(() => (mark_id = null))
                          "
            />
        ...
        <v-col cols="12" md="12" sm="12">
          ...
          >
            <template v-slot:body>
              <tbody>
              <template v-for="(item, i) in models">
                <tr
                  :key="'row-' + item.id"
                ...
                >
                  ...
                  <td>
                    <template v-if="item.created_at">{{ $moment(item.created_at).format('DD.MM.YYYY') }}</template>

                  </td>
                  <td>
                    <template v-if="item.created_at">{{ $moment(item.updated_at).format('DD.MM.YYYY') }}</template>
                  </td>
                </tr>
              </template>
              </tbody>
...
export default {
  ...
  computed: {
    marks() {
      return this.$store.state.admin.marks
    },
    models() {
      if (this.search) {
        return this.$store.state.admin.models.filter(model => model.name.toLowerCase().includes(this.search.toLowerCase()))
      }
      else return this.$store.state.admin.models
    },
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.valid) {
        ...
        if (this.editedIndex > -1) {
          this.$store
            .dispatch('admin/updateModel', {
              item: this.editedItem
            }).then((res) => {
            this.$toast.success("Модель изменена!", {
              position: "top-right",
              timeout: 2000,
              closeOnClick: true,
          ...
          this.$store
            .dispatch('admin/saveModel', {
              item: this.editedItem
            }).then((res) => {
            this.$toast.success("Модель добавлена!", {
              position: "top-right",
              timeout: 2000,
              closeOnClick: true,
              pauseOnFocusLoss: true,
    ...
    deleteItem() {
      this.$store
        .dispatch('admin/deleteModel', {
          id: this.deleteId
        })
        .then(() => {
          this.$toast.success("Модель удалена", {
            position: "top-right",
            timeout: 2000,
            closeOnClick: true,
            pauseOnFocusLoss: true,
    ...
    getModels (id = null) {
      if (id > 0) {
        this.$store.dispatch('admin/fetchModels', { id })
      }
    },
  }
}
