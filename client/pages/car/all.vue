          ...
          >
            ...
            <template v-slot:top>
              <v-toolbar flat dense>
                <v-row dense class="pt-2 mt-4 hidden-sm-and-down">
                  ...
                  <v-col cols="4" md="3" xs="6">
                    <v-row dense>
                      ...
                      <v-col
                        ...
                        xs="6"
                        class="mt-0"
                        v-if="!$checkAccess('download', 'cars/all')"
                      >
                        <v-btn
                          class="mt-n1"
                          icon
...
export default {
  ...
  mounted() {
    this.$echo.channel('cars').listen('CarAdded', (e) => {
      ...
      this.$store
        .dispatch('car/fetchCars', { showroom_id: [1, 2, 4, 5, 7, 8, 10, 11] })
        .catch((error) => {
          console.log(error)
          this.reload()
        })
  ...
  methods: {
    ...
    canSee() {
      const users = ['admin', 'custom', 'director', 'petr']
      return users.includes(this.role?.name)
    },
    canChange() {
      const users = [
    ...
    hasAccess() {
      const roles = ['admin', 'director', 'coordinator']
    ...
    exportFile() {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Новые авто')
      let wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' })
      saveAs(
        new Blob([wbout], { type: 'application/octet-stream' }),
        'Список_всех_новых_машин.xlsx'
      )
    },
    getStatus(value) {
      if (value === 1) {
