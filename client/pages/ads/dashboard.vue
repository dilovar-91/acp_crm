      ...
      <v-row no-gutters align="start" class="d-flex">
        <v-col cols="12">
          <v-app-bar dense class="grey lighten-5" elevate-on-scroll>
            ...
            <v-btn color="pink" dark class="my-2 ml-2"  @click="applyDateRange('tomorrow')">
              Завтра
            </v-btn>

            <v-btn color="primary" dark class="my-2 ml-2"  @click="applyDateRange('today')">
              Сегодня
            </v-btn>
            <v-btn color="warning" dark class="my-2 ml-2"  @click="applyDateRange('yesterday')">
              Вчера
            </v-btn>
            <v-btn color="black" dark class="my-2 ml-2"  @click="applyDateRange('week')">
              Неделя
            </v-btn>
            <v-btn color="red" dark class="my-2 ml-2"  @click="applyDateRange('month')">
              Месяц
            </v-btn>
                      ...
                      <v-card-text class="pa-4 ">
                        <v-row>
                          <v-col cols="6" lg="3" md="3" style="padding: 4px !important;">
                            <DashboardCard2
                              :to="'/tablet/arrivals/1'" title="Новые"
                              :value="computeCount('arrivals', 1)" :subtitle="'('+computeCount('arrivals_credit', 1) + '/'+computeCount('arrivals_nal', 1) + ')'" icon="mdi-new-box"
                            />
                          </v-col>
                          <v-col cols="6" lg="2" md="2" style="padding: 4px !important;">
                            <DashboardCard2
                          ...
                          <v-col cols="6" lg="2" md="2" style="padding: 4px !important;">
                            <DashboardCard2
                              :to="'/tablet/arrivals/1'" title="Всего"
                              :value="computeCount('not_coming', 1)" :subtitle="'('+computeCount('not_coming_credit', 1) + '/'+computeCount('not_coming_nal', 1) + ')'"  icon="mdi-handshake"
                            />
                          </v-col>
                          <v-col cols="6" lg="3" md="3" style="padding: 4px !important;">
                            <DashboardCard2
                              :to="'/tablet/arrivals/1'" title="Эффективность"
                              :value="computeCount('not_answering', 1)" icon="mdi-label-percent"
                            />
                          </v-col>
                          ...
                          <v-col cols="6" lg="3" md="3" style="padding: 4px !important;">
                            <DashboardCard2
                              :to="'/tablet/arrivals/1'" title="Продажа"
                              :value="computeCount('arrivals', 1)" :subtitle="'('+computeCount('arrivals_credit', 1) + '/'+computeCount('arrivals_nal', 1) + ')'" icon="mdi-contactless-payment-circle-outline"
                            />
                          </v-col>
                          <v-col cols="6" lg="3" md="3" style="padding: 4px !important;">
                            <DashboardCard2
                              :to="'/tablet/arrivals/1'" title="Соскок"
                              :value="computeCount('will_arrive', 1)" icon="mdi-run-fast" :subtitle="'('+computeCount('will_arrive_credit', 1) + '/'+computeCount('will_arrive_nal', 1) + ')'"
                            />
                          </v-col>

                          <v-col cols="6" lg="3" md="3" style="padding: 4px !important;">
                            <DashboardCard2
                              :to="'/tablet/arrivals/1'" title="Расход"
                              :value="computeCount('arrived', 1)" :subtitle="'('+computeCount('arrived_credit', 1) + '/'+computeCount('arrived_nal', 1) + ')'"  icon="mdi-exit-run"
                            />
                          </v-col>
                          <v-col cols="6" lg="3" md="3" style="padding: 4px !important;">
                            <DashboardCard2
                              :to="'/tablet/arrivals/1'" title="Цена продажи"
                              :value="computeCount('not_coming', 1)" :subtitle="'('+computeCount('not_coming_credit', 1) + '/'+computeCount('not_coming_nal', 1) + ')'"  icon="mdi-handshake"
                            />
                          </v-col>
                          ...
                          <v-col cols="6" lg="6" md="6" style="padding: 4px !important;">
                            <DashboardCard2
                              :to="'/cars/1'" title="Приехал сегодня"
                              :value="computeCount('cars', 1)" icon="mdi-home-account"
                              :color="'warning'"
                            />
                          </v-col>
                          <v-col cols="6" lg="6" md="6" style="padding: 4px !important;">
                            <DashboardCard2
                              :to="'/used-cars/1'" title="Приедет завтра"
                              :value="computeCount('usedCars', 1)" icon="mdi-home-alert"
                              :color="'warning'"
                            />
                          </v-col>
                        </v-row>
...
export default {
  ...
  methods: {
    ...
    async setDateRange(from, to) {
      ...
      this.filter_to = to;
      await this.$store.dispatch('dashboard/fetchOrders', {
    ...
    applyDateRange(rangeType) {
      ...
      switch (rangeType) {
        case 'tomorrow':
          from = this.$moment().add(1, 'days').format('YYYY-MM-DD');
          to = from;
          break;
        case 'today':
    ...
    async filter () {
      if (this.filter_from && this.filter_to) {
        await this.$store.dispatch('dashboard/fetchOrders', { dateFrom: null, dateTo: null })
      }
    },
    async reset () {
