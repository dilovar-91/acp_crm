...
export default {
  ...
  methods: {
    ...
    getLink(item, status = null, agency_id = null, payment_id = null, approved = null) {
      ...
      return 'https://' + window.location.hostname + '/crm/' + item.showroom_id + '/orders-mini/'+item.agency_id+'?' + status_query + agency_query + payment_query + approved_query + '&from=' + this.$moment(this.date_from, 'DD.MM.YYYY HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') + '&to=' + this.$moment(this.date_to, 'DD.MM.YYYY HH:mm:ss').format('YYYY-MM-DD HH:mm:ss')
    },
    groupedData() {
