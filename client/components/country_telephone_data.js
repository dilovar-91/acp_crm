...
export const findFirstCountryByCode = code => {
  const filteredCode = code.match(/\d/g).join("");
  const filteredCountries = allCountries.filter(
    contryArr => contryArr[2] === filteredCode && contryArr[3]
  );

  const country = filteredCountries[0] ? filteredCountries[0][1] : "";
...
export const findFirstCountryByAlpha2 = alpha2 => {
  ...
  const country = filteredCountries[0] ? { country: filteredCountries[0][1], code: filteredCountries[0][2] } : null;
...
export const getMaskToLibPhoneNumber = libPhoneNumberObj => {
  ...
  const mask = filteredCountries[0][3]
...
export const isCanada = number => {
  const filteredNumber = number.match(/\d/g).join("");
